...
class MyCompoundBody : public MyRigidBody
{
protected:
    struct ChildBody{
        ChildBody(){}
        ChildBody(const PositionInfo& t, MyRigidBody* b, real_t m)
...
public:
    //coordinat base the compound body's origin
     void addBody(real_t mass, MyRigidBody* body, real_t x=0, real_t y=0, real_t z=0, real_t yaw=0, real_t pitch=0, real_t roll=0l){
        addBody(body, PositionInfo(x, y, z, yaw, pitch, roll), mass);
    }

    void addBody(MyRigidBody* body, const PositionInfo& origin, real_t mass=0.f){
