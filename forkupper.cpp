    ...
    :MyCompoundBody(mass)
{
    ...
    _x = width+8.f;
    _z = 4.f;
    _y =  crown_height + travel + steer_len;

    _left_tube = new MyCylinder(1.8f, travel, 1.8f);
    _left_tube->color(0,255,0);
    addBody( cmass, _left_tube, -(width/2.f+1.8f), -(_y/2.f-travel/2.f), 0.f);

    _right_tube = new MyCylinder(1.8f, travel, 1.8f);
    _right_tube->color(0, 255, 0);
   addBody(cmass,  _right_tube,  width/2.f+1.8f, -(_y/2.f-travel/2.f), 0.f);

    _crown = new MyBox(width+8.f, crown_height, crown_height);
    _crown->color(255,0,0);
    addBody(cmass,_crown, 0.f, (travel+crown_height/2.f)-_y/2.f, 0.f);
